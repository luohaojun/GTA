...
void callback(const sensor_msgs::CompressedImage::ConstPtr &rgbimage ,const sensor_msgs::PointCloud2::ConstPtr &pcl)
{
    // cv::Mat visImg;    
        ...
        if(!frame.empty())
        {
            ...
            if(temp.size()!=0)
            {
                ...
                if(potential.size()!=0)
                {
                    ...
                    cv::Mat X(4, 1, cv::DataType<double>::type);
                    cv::Mat Y(3, 1, cv::DataType<double>::type);
                    int depthbox_w = potential[maxElementIndex].boundingbox.width * 0.35;
