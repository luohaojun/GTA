...
static double fx, fy, cx, cy; //focal length and run_yoloprincipal point
...
void callback(const sensor_msgs::CompressedImageConstPtr & rgbimage, const sensor_msgs::ImageConstPtr & depth)
{
    ...
    // cout<<"time stamp of /obj_pose_cam: "<<send.header.stamp<<endl;
    ...
    while(ros::ok())
    {
        ...
        if(!frame.empty())
        {
            ...
            if (found)
            {
                ...
                // ofstream save("/home/luo/gta/src/AUTO/offb/src/include/yolo/gt_v7_test1.txt", ios::app);
                // save<<counter<<endl;
